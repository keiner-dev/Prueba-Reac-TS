...
export default function EventForm({ categories, presetCategoryId, initialEvent, onSubmit }: EventFormProps) {
  const [name, setName] = useState(initialEvent?.name ?? "");
  const [description, setDescription] = useState(initialEvent?.description ?? "");
  const [date, setDate] = useState(initialEvent?.date ?? "");
  const [location, setLocation] = useState(initialEvent?.location ?? "");
  const [price, setPrice] = useState(initialEvent?.price?.toString() ?? "");
  const [capacity, setCapacity] = useState(initialEvent?.capacity?.toString() ?? "");
  const [categoryId, setCategoryId] = useState(initialEvent?.categoryId ?? presetCategoryId ?? "");
  const [imagesText, setImagesText] = useState(initialEvent?.images?.join("\n") ?? "");
  ...
  return (
    ...
    >
      <h2 className="text-xl font-bold">{initialEvent ? "Editar evento" : "Crear evento"}</h2>
      ...
      >
        {submitting ? "Guardando…" : initialEvent ? "Guardar cambios" : "Crear evento"}
      </button>
    </form>
  );
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      await onSubmit(payload, !!initialEvent, initialEvent?.id);
    } catch (err) {
